...
# Implementation of Deep Q-Network (DQN) algorithm is inspired from PyTorch's tutorial: https://pytorch.org/tutorials/intermediate/reinforcement_q_learning.html
...
class DQNScheduler(Scheduler):
    ...
    def __init__(self, corpora_infos, nb_actions, nb_states, opts, device_id) -> None:
        ...
        self.last_state = torch.tensor(
            default_state, dtype=torch.float32, device=self.device
        ).unsqueeze(0)
        self.memory = ReplayMemory(self.max_replay_capactiy)
        self.exploration = True
        self.Q = None
        self.policy_net = DQN(self.nb_states, self.nb_actions, self.hidden_size).to(
            self.device
        )
        self.target_net = DQN(self.nb_states, self.nb_actions, self.hidden_size).to(
            self.device
        )
    ...
    def _parse_opts(self):
        ...
        self.max_replay_capactiy = self.opts.dqn_max_replay_capacity
    ...
    def select_action(self, step, state):
        ...
        if sample > eps_threshold:
            ...
            self.Q = None
            return torch.tensor(
                [[random.randint(0, self.nb_actions - 1)]],
                device=self.device,
                dtype=torch.long,
            )
    ...
    def optimize_model(self):
        if (
            ...
            or len(self.memory) < self.min_replay_capacity
        ):
            return
            ...
            if step >= self.warmup_steps:
                ...
                if len(self.memory) >= self.min_replay_capacity:
                    ...
                    self.action = torch.tensor(
                        [[np.random.choice(self.nb_actions)]],
                        device=self.device,
                        dtype=torch.long,
                    )
            else:
    ...
    def _log(self, step):
        ...
        if self.Q is not None:
            ...
            qvalues += "]"
        logger.info(
            f"Step:{step+1};GPU:{self.device_id};Q-values:{qvalues};Action:{self.action.item()+1};Delta-reward:{self.delta_reward};Exploration:{self.exploration}"
        )
